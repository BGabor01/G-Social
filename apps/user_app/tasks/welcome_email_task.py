...
def send_welcome_email(email: str | bytes):
    ...
    """
    try:
        welcome_rpc_client = Client('EMAIL_SENDER', host=os.environ.get('RABBITMQ_HOST', 'localhost'), username=os.environ.get(
            'RABBITMQ_USER', 'guest'), password=os.environ.get('RABBITMQ_PASS', 'guest'))
